...
// layer (WebSocket channel on 7901) into registered control surfaces. The UI
// is the Foldkit app in src/ui, served by Vite, which
...
import { ControlSurfaces } from "./control/surface";
import { makeMacSurface } from "./control/surfaces/mac/overlay";

const DEFS = `${import.meta.dir}/gesture/defs`;

export class DefsLoadError extends Schema.TaggedErrorClass<DefsLoadError>()("DefsLoadError", {
  file: Schema.String,
  cause: Schema.Defect,
}) {}
...
const program = Effect.gen(function* () {
                ...
                loadDefs().pipe(
                  ...
                  Effect.catchTag("DefsLoadError", (err) =>
                    Effect.logError("Failed to load gesture defs", err).pipe(
                      Effect.map(() =>
                        Response.json({ error: `failed to load ${err.file}` }, { status: 500 }),
                      ),
                    ),
                  ),
                ),
  ...
  yield* Effect.log("Note: clicking requires Accessibility permission for your terminal app");
  // The bridge runs for the life of the app (its event stream never ends)
  return yield* runControlBridge;
});

const controlSurfacesLayer = Layer.effect(
  ControlSurfaces,
  Effect.gen(function* () {
    const mac = yield* makeMacSurface;
    return { surfaces: [mac] };
  }),
);

const controlLayer = Layer.merge(
  webSocketChannelLayer.pipe(Layer.provide(BunHttpServer.layerServer({ port: 7901 }))),
  controlSurfacesLayer,
);
