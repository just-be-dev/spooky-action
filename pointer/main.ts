...
// over WebSocket to the native macOS overlay (overlay.swift), which draws
// the on-screen ring and posts real clicks.
...
const server = Bun.serve({
  ...
  websocket: {
    ...
    message(_ws, raw) {
      ...
      switch (msg.t) {
        case "circle":
          command(`circle ${msg.x} ${msg.y} ${msg.r}`);
          break;
        case "hide":
          command("hide");
          break;
    ...
    close() {
      command("hide"); // tracker tab closed — don't leave a stale ring up
    },
  },
});
